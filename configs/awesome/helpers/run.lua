...
function _run.run_once_ps(findme, cmd)
	awful.spawn.easy_async_with_shell(string.format("ps -C %s|wc -l", findme),
		function(stdout)
			if tonumber(stdout) ~= 2 then awful.spawn(cmd, false) end
		end)
end
...
function _run.run_once_grep(command)
	awful.spawn.easy_async_with_shell(string.format(
		...
		if stdout == "" or stdout == nil then awful.spawn(command, false) end
	end)
end
...
function _run.check_if_running(command, running_callback, not_running_callback)
	awful.spawn.easy_async_with_shell(string.format(
		...
		if stdout == "" or stdout == nil then
			...
			if running_callback ~= nil then running_callback() end
		end
	end)
end
